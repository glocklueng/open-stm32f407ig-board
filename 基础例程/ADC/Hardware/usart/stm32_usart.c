...
static struct stm32_usart *stm32_usart_ops(USART_TypeDef* USARTx,uint32_t txrx)
{
	 ...
	 stm32_usart_unit.gpio_rx=(GPIO_TypeDef *)(((txrx&0x0000f000)>>4)+GPIOA_BASE);
...
void stm32_usart_dma(USART_TypeDef* USARTx,DMA_Stream_TypeDef* DMAy_Streamx,uint8_t message,uint8_t *dmabuf,uint32_t DMA_IT,uint8_t USART_IRQn)
{
	...
	DMAy_Streamx->CR|=(1<<8);                //circular mode enable
	DMAy_Streamx->CR|=0<<6;                //Peripheral to Memory,[7,6]->00 ptom ,01 mtp, 11 mtm
	DMAy_Streamx->FCR=0x00000000;
	
	DMAy_Streamx->NDTR=500;                //Number of data items to transfer
	...
	{
		{
			...
			NVIC_InitStructure.NVIC_IRQChannel = USART6_RX_IRQ;
