...
{  							  
  u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到一个字符点阵的总字节数
	//设置窗口		   
	num=num-' ';//得到偏移后的值
	for(t=0;t<csize;t++)
	{   
...
static char lcdshowbuf[26][40];
//static char lcdbackbuf[26][40];
extern int vsnprintf(char *str, unsigned int size,  const  char  *format,  va_list ap);
va_list arg_ptra;

void lcd_showlongstring(void)
{
 	 u16 x=0;
	 u16 i,j;
	 for(i=0;i<26;i++)
	 {
		 for(j=0;j<40;j++)
		 {
			 if( (lcdshowbuf[i][j]<='~')&&(lcdshowbuf[i][j]>=' ') )
         LCD_ShowChar(x,i*12,lcdshowbuf[i][j],12,0);
			 x+=6;
		 }
		 x=0;
   }
}
void lcd_get_someline(unsigned int len)
{
   u16 i,j;
// 	for(i=0;i<len;i++)   //backup
// 	{
//     for(j=0;j<40;j++)
// 		 lcdbackbuf[i][j] = lcdshowbuf[i][j];
//   }
	for(i=0;i<(26-len);i++)
	{
    for(j=0;j<40;j++)
		  lcdshowbuf[i][j] = lcdshowbuf[i+len][j];
  }
// 	for(i=0;i<len;i++)   
// 	{
//     for(j=0;j<40;j++)
// 		 lcdshowbuf[26-len+i][j] = lcdbackbuf[i][j];
//   }
	
}

void lcd_data_convert(const char *in,u8 inlen)
{
	static u16 x=0,y=0;
	u16 cnt=0,i;
...
{
	static char lcddatabuf[CMD_BUFFER_LEN];
	static u16 len;
	
  va_start(arg_ptra,data);
  vsnprintf(lcddatabuf, CMD_BUFFER_LEN+1, data, arg_ptra);
	len = strlen(lcddatabuf);
	
	lcd_data_convert(lcddatabuf,len);
